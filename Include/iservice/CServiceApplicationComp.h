...
public:
	...
	*/
	...
	I_BEGIN_COMPONENT(CServiceApplicationComp);
		...
		I_ASSIGN(m_failureResetPeriodMinutesAttrPtr, "FailureResetPeriodMinutes", "Minutes after which the service failure counter is reset (min 0)", false, 10);
		...
		I_ASSIGN_MULTI_3(m_failureRetryDelaysSecAttrPtr, "FailureRetryDelaysSeconds", "Delay in seconds before the recovery action for the 1st, 2nd and 3rd+ failures (min 0)", false, 3, 8, 30);
	I_END_COMPONENT;
